                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div
                                    ...
                                    <h3 class="marginright">In-{{ $medicine->id }}</h3>
                                    <span
                                        class="marginright">{{ \Carbon\Carbon::parse($medicine->created_at)->translatedFormat('l j F Y H:i') }}</span>
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                ...
                                <div class="col-sm-3">
                                    <p class="lead marginbottom"> @lang('hometr.Medicine bill') <br></p>
                                </div>

                                <div class="col-sm-3 text-right payment-details">
                                    <p class="lead marginbottom payment-info">@lang('hometr.Details')</p>
                                    <p>@lang('hometr.Date'):
                                        {{ \Carbon\Carbon::parse($medicine->created_at)->translatedFormat('l j F Y H:i') }}
                                    </p>
                                    <p>@lang('hometr.Total salary'): {{ $medicine->salary }}
                                        @lang('hometr.pound')</p>
                                </div>
                            </div>
                                ...
                                <table class="table table-striped">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-right" style="width:15%">@lang('hometr.Medicine name')</th>
                                            <th class="text-right" style="width:15%">@lang('hometr.Medicine salary')</th>
                                            <th class="text-right" style="width:15%">@lang('hometr.Buy date')</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td class="text-center">1</td>
                                            <td class="text-right">{{ $medicine->name }}</td>
                                            <td class="text-right">{{ $medicine->salary }} @lang('hometr.pound')
                                            </td>
                                            <td class="text-right">
                                                {{\Carbon\Carbon::parse($medicine->created_at)->translatedFormat('l j F Y H:i') }} 
                                                </td>
        ...
        $(function() {
            $("#btnPrint").click(function() {
                ...
                frameDoc.document.write(
                    '<html lang="{{ LaravelLocalization::getCurrentLocale() }}" dir="{{ LaravelLocalization::getCurrentLocaleDirection() }}"><head><title>Re-{{ $medicine->id }}</title>');
